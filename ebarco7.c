...
void swapValueReturnedWExpected(int *pointerA, int *pointerB, int *pointerC, int *pointerD);
int main(void)
{
...
//these print statment print the same thing because we set our pointer to
//point to expected
...
//it also returns a value to we need to save that value after the function
//completes
...
swapValueReturnedWExpected(&valueReturned,&expected, &swapVar1, &swapVar2 );

printf("After swapping: expected = %d, valueReturned = %d \n", expected, valueReturned);
printf("After swapping: var 1 = %d, var2 = %d \n", swapVar1, swapVar2);
return EXIT_SUCCESS;
}
int compare_and_swap(int *pointb, int expected, int new_value)
{
//we can use the same name for expected and it will be a different variable
//in this function.
int temp = *pointb;
//created a temp variable to store the original value located at the pointer
//pointb
if(*pointb == expected)
{
*pointb = new_value;
